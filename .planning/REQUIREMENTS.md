...
- [x] **DRAFT-03**: Mode-B drafter uses Anthropic SDK (Opus 4.7) with voice samples in-context and prompt caching enabled (ephemeral `ttl="1h"`); per-scene opt-in via controller decision; scene carries `mode="B"` in event log and commit metadata.
- [x] **DRAFT-04**: Structurally complex beats (Cholula stir, two-thirds revelation, siege climax — list derived from outline tags) are pre-flagged for Mode-B by default and can be demoted to Mode-A via config.
...
- [x] **PHYSICS-01**: Pydantic `SceneMetadata` model implements all D-03 mandatory fields + D-13 ownership fields + D-04 staging fields. Strict validation (`extra="forbid"`); enforced via `pydantic.ValidationError` at stub load time.
...
- [x] **PHYSICS-10**: `scene_buffer_similarity` axis: BGE-M3 cosine ≥0.80 vs prior committed scenes' embeddings. Embedding cache lives at `.planning/intel/scene_embeddings.sqlite`.
- [x] **PHYSICS-11**: Quote-corruption `., ` defensive normalizer in `chapter_assembler/concat.py` (D-18 PHYSICS-11).
- [x] **PHYSICS-12**: ch15+ first-flight smoke: ch15 sc02 resume passes all 13 axes (or scene-kicks recover deterministically). ch01-04 read-only smoke: engine flags zero false positives on the 4 frozen-baseline chapters.
- [x] **PHYSICS-13
...
| DRAFT-03 | Phase 5 | Complete |
| DRAFT-04 | Phase 5 | Complete |
